...
FALLBACK_MJCF = {
    "agile_one": Path("/home/ruiming.wu/codes/H4/mjcf/agile_one_no_hands.xml"),
}
...
def load_motion(path: Path, robot_arg: str, fps_arg: float | None) -> tuple[str, str, np.ndarray, np.ndarray, np.ndarray, float]:
    """Load motion and return name, robot, root_pos, root_quat_xyzw, dof_rad, fps."""
    ...
    if suffix == ".csv":
        ...
        return path.stem, robot, root_pos, root_quat_xyzw, dof_rad, fps
    ...
    if suffix == ".pkl":
        ...
        return motion_name, robot, root_pos, root_quat_xyzw, dof_rad, fps
...
def set_camera(cam: mujoco.MjvCamera, qpos: np.ndarray, frame: int, follow: bool) -> None:
    root = qpos[frame, 0:3]
...
def render_motion(
    ...
    frame_indices = list(range(0, qpos.shape[0], max(1, stride)))
    ...
    try:
        for out_i, frame in enumerate(frame_indices):
            ...
            set_camera(cam, qpos, frame, follow=follow)
...
def main() -> None:
    ...
    motion_name, robot, root_pos, root_quat_xyzw, dof_rad, source_fps = load_motion(
        motion_path, args.robot, args.fps
    )
    if dof_rad.shape[1] != 29:
        raise ValueError(f"Expected 29 DOFs, got {dof_rad.shape[1]}")

    qpos = make_qpos(root_pos, root_quat_xyzw, dof_rad)
